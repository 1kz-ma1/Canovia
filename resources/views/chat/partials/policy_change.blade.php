...
@if ($step === 'prompt' && $selectedPlan)
    <div class="assistant-message-row assistant-message-left">
        ...
        <div class="assistant-bubble assistant-bubble-support assistant-wide-bubble">
            ...
            <textarea id="chatPolicyChangePrompt" readonly tabindex="-1" aria-hidden="true" class="sr-only">{{ $answers['prompt'] }}</textarea>
            ...
            <div class="mt-4 rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.035] p-4">
                ...
                <div class="mt-4 flex flex-wrap gap-3">
                    <button type="button" class="btn-primary" data-copy-target="#chatPolicyChangePrompt">プロンプトをコピー</button>
        ...
        <div class="assistant-bubble assistant-bubble-user assistant-form-bubble">
            ...
            >
                ...
                <button
                    ...
                    data-paste-target="#chat_policy_operations_json"
                    data-paste-submit="1"
                    data-paste-fallback="#chatPolicyOperationsJsonManual"
                    data-paste-status="#chatPolicyOperationsJsonPasteStatus"
                >クリップボードから貼り付けて確認</button>
                <p id="chatPolicyOperationsJsonPasteStatus" class="hidden text-xs leading-5 text-slate-400" aria-live="polite"></p>

                <details id="chatPolicyOperationsJsonManual" class="ai-handoff-details rounded-xl border border-slate-800 bg-slate-950/35 p-3" @if($errors->has('operations_json') || old('operations_json')) open @endif>
                    <summary class="cursor-pointer text-xs font-semibold text-slate-300">手動で貼り付ける / JSONを確認する</summary>
                    <textarea
                        id="chat_policy_operations_json"
